        ...
        INTEGER, PARAMETER :: N = 80, k = 3, ITERATIONS = 1000, D = 1
        ...
        U_new = 0.0
        IF (TYPE_III) THEN
                CALL GET_LHS_TYPE_III(N, alpha, theta, dt, r_arr, phi_arr, k, I0, sigma, A_mat)
                CALL DSOLVE(1, A_mat, U_new, N**2, N, N**2, 2*N+1)

                CALL GET_RHS_TYPE_III(N, alpha, theta, dt, r_arr, phi_arr, k, I0, sigma, U, U_new)
                U = U_new

                WRITE(filename, '("output/u_transient_3_", I3.3, "_", ES7.1, "_", ES7.1, "_", I1, ".dat")') N, theta, rich, D
                OPEN(1, file=filename)
                ...
                DO i=1,ITERATIONS
                        CALL GET_RHS_TYPE_III(N, alpha, theta, dt, r_arr, phi_arr, k, I0, sigma, U, U_new)
                        CALL DSOLVE(2, A_mat, U_new, N**2, N, N**2, 2*N+1)
                        U = U_new
                ...
                CLOSE(1)
        ELSE
                CALL GET_LHS_TYPE_I(N, theta, dt, r_arr, phi_arr, k, I0, sigma, A_mat)
                CALL DSOLVE(1, A_mat, U_new, N**2, N, N**2, 2*N+1)

                CALL GET_RHS_TYPE_I(N, theta, dt, r_arr, phi_arr, k, I0, sigma, U, U_new)
                U = U_new

                WRITE(filename, '("output/u_transient_1_", I3.3, "_", ES7.1, "_", ES7.1, "_", I1, ".dat")') N, theta, rich, D
                OPEN(1, file=filename)
