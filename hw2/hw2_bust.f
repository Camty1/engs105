        ...
        REAL*8, PARAMETER :: THRESHOLD=0.00001
        INTEGER :: N, i, j, k, row, col, new_col, width, counter
        REAL*8 :: r_min, r_max, theta_min, theta_max, delta_r, delta_theta
        REAL*8 :: I0, sigma, theta, alpha, temp
        REAL*8 :: A, B, C, D, E, f, g, err
        REAL*8, DIMENSION(:), ALLOCATABLE :: r_array, theta_array, U, Y
        REAL*8, DIMENSION(:), ALLOCATABLE :: U_true
        REAL*8, DIMENSION(:), ALLOCATABLE :: diag
        REAL*8, DIMENSION(:, :), ALLOCATABLE :: coeff_mat, low_up
        ...
        low_up(:, width+1) = 0.0
        DO i=1,N**2
                diag(i) = coeff_mat(i, width+1)
        END DO
        
        OPEN(1, file="truth/u100_1.dat")
        counter = 1
        DO i=1,100
        DO j=1,100
        if (MOD((i-1)*100+j-1, 100/N) == 0) THEN
                READ(1,'(A)') U_true(counter)
                PRINT*, U_true(counter)
                counter = counter + 1
        ELSE
                READ(1,'(A)')
        END IF
        ...
        WRITE(fname, '( "output/jacobi", I3.3, ".dat" )' ) N
        OPEN(1, file=fname)
        WRITE(fname, '( "output/jacobi_err", I3.3, ".dat" )' ) N
        OPEN(2, file=fname)
...
!                PRINT *, i, err
!                CALL JACOBI(diag, low_up, Y, N**2, width, U)
!                CALL CALC_ERROR(U, U_TRUE, N**2, err)
!                WRITE(2,*) err
!                i = i + 1
!        END DO
